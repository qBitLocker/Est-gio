...
class CipherTools():
    ...
    # Implementação do Bcrypt - Algoritmo de hash de senha baseado em Blowfish
    def Bcrypt_enc(self, plaintext: bytes) -> bytes:
        if not self.__check_bytes__(plaintext):
            raise ValueError("[ERROR] Tipo de dado inválido para plaintext")
    ...
    def Bcrypt_match(self, ciphertext: bytes, hash: bytes) -> bool:
        if self.__check_bytes__(ciphertext) and self.__check_bytes__(hash):
            ...
            raise ValueError("[ERROR] Tipo de dado inválido para ciphertext ou hash")
